"""Inference-only Phi-tiny-MoE-instruct model (SlimMoE architecture)."""
...
class PhiMoE(nn.Module):
    ...
    def forward(self, index:int, hidden_states: torch.Tensor, experts_cache: torch.Tensor, 
        ...
        if (input_metadata is not None and 
            ...
            # Use standard stack_fused_moe instead of custom implementation
    ...
    def _stack_fused_moe_with_routing_info(self, hidden_states, w1, experts_cache, gating_output, topk, renormalize, inplace):
        ...
        from fastmoe.layers.stacked_fused_moe import moe_align_block_size, invoke_fused_moe_kernel, silu_and_mul
...
class PhiAttention(nn.Module):
    ...
    def __init__(self,
                 ...
                 attention_bias: bool = False) -> None:
        super().__init__()
        self.hidden_size = hidden_size
        ...
        # Use explicit head_dim from config if provided, otherwise calculate from hidden_size
        ...
        # Use rope_parameters dict format for compatibility with newer vLLM
...
class PhiDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        # Get head_dim from config if available (Phi-tiny-MoE has explicit head_dim)
        head_dim = getattr(config, "head_dim", None)
        # Phi-tiny-MoE (SlimMoE) typically has attention_bias=False
        attention_bias = getattr(config, "attention_bias", False)
        # Handle sliding_window - may not exist in all configs
...
class PhiMoEForCausalLMOff(nn.Module):
    ...
    def __init__(
        ...
        self.offload_device = offload_device
        self.model = PhiModel(config, linear_method, offload_device=offload_device)
        # Phi-tiny-MoE (SlimMoE) has lm_head_bias=True
        lm_head_bias = getattr(config, "lm_head_bias", False)
        self.lm_head = ParallelLMHead(config.vocab_size, config.hidden_size, bias=lm_head_bias)
...
# Distinct class name for Phi-tiny-MoE to avoid confusion with Phi-3.5-MoE
class PhiTinyMoEForCausalLMOff(PhiMoEForCausalLMOff):
    """Phi-tiny-MoE-instruct model (SlimMoE architecture)."""
    pass

EntryClass = PhiTinyMoEForCausalLMOff