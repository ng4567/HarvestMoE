...
# Test script to run Qwen3-30B-A3B benchmarks in 2-GPU and 4-GPU modes using simple server
...
echo "Starting Qwen3-30B-A3B benchmarks (test mode)..."
...
start_server() {
    ...
    cd "$PROJECT_ROOT"
    python fastmoe/serve/launch_server_simple.py \
...
run_benchmark() {
    ...
    # Run benchmark
    cd "$PROJECT_ROOT"
    ./benchmarks/mtbench/benchmark.sh localhost $PORT 5
    ...
    if [ -f "benchmarks/mtbench/bench_results.csv" ]; then
        ...
        head -5 "benchmarks/mtbench/$result_file"
    else
...
for result_file in benchmarks/mtbench/qwen3_30b_a3b_*gpu_results.csv; do
    if [ -f "$result_file" ]; then
        ...
        if command -v bc >/dev/null 2>&1; then
            avg_latency=$(tail -n +2 "$result_file" | cut -d',' -f5 | awk '{sum+=$1} END {print sum/NR}')
            avg_throughput=$(tail -n +2 "$result_file" | cut -d',' -f6 | awk '{sum+=$1} END {print sum/NR}')
